            ...
            do {
                ...
                switch(thread.opcode) {
                    ...
                    case 0x07: // jump u16
                        op_jump(thread);
                        break;
                    case 0x08: // start thread:u08,u16
                        op_start(thread);
                        break;
                    case 0x09: // dbra $[x],u16
                        op_dbra(thread);
                        break;
                    case 0x0a: // cjmp cond:u08,$[x],y
                        op_cjmp(thread);
                        break;
                    case 0x0b: // fade pal:u16
                        op_fade(thread);
                        break;
                    ...
                    case 0x15: // iori [$x],u16
                        op_iori(thread);
                        break;
                    case 0x16: // lsli [$x],u16
                        op_lsli(thread);
                        break;
                    case 0x17: // lsri [$x],u16
                        op_lsri(thread);
                        break;
                    case 0x18: // sound id:u16,frequency:u08,volume:u08,channel:u08
                        op_sound(thread);
                        break;
                    case 0x19: // load id:u16
                        op_load(thread);
                        break;
...
auto VirtualMachine::op_setr(Thread& thread) -> void
{
    const uint8_t dst = _bytecode.fetchByte();
    const uint8_t src = _bytecode.fetchByte();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s [$%02x],[$%02x]", thread.thread_id, thread.current_pc, "setr", dst, src);

    _registers[dst].u = _registers[src].u;
}

auto VirtualMachine::op_seti(Thread& thread) -> void
{
    const uint8_t  dst = _bytecode.fetchByte();
    const uint16_t imm = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s [$%02x],$%04x", thread.thread_id, thread.current_pc, "seti", dst, imm);
...
auto VirtualMachine::op_lsli(Thread& thread) -> void
{
    const uint8_t  dst = _bytecode.fetchByte();
    const uint16_t imm = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s [$%02x],%d", thread.thread_id, thread.current_pc, "lsli", dst, imm);

    _registers[dst].u <<= imm;
}

auto VirtualMachine::op_lsri(Thread& thread) -> void
{
    const uint8_t  dst = _bytecode.fetchByte();
    const uint16_t imm = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s [$%02x],%d", thread.thread_id, thread.current_pc, "lsri", dst, imm);
...
auto VirtualMachine::op_dbra(Thread& thread) -> void
{
    const uint8_t  dst = _bytecode.fetchByte();
    const uint16_t imm = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s [$%02x],$%04x", thread.thread_id, thread.current_pc, "dbra", dst, imm);
...
auto VirtualMachine::op_start(Thread& thread) -> void
{
    ...
    LOG_DEBUG("|$%02x|$%04x|\t%-7s $%02x,$%04x", thread.thread_id, thread.current_pc, "start", tid, imm);
...
auto VirtualMachine::op_halt(Thread& thread) -> void
{
    LOG_DEBUG("|$%02x|$%04x|\t%-7s (halt this thread)", thread.thread_id, thread.current_pc, "halt");
...
auto VirtualMachine::op_load(Thread& thread) -> void
{
    const uint16_t id = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "load", id);

    _engine.loadResource(id);
}

auto VirtualMachine::op_fade(Thread& thread) -> void
{
    const uint16_t palette = _bytecode.fetchWord();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s $%04x", thread.thread_id, thread.current_pc, "fade", palette);
...
auto VirtualMachine::op_show(Thread& thread) -> void
{
    const uint8_t src = _bytecode.fetchByte();

    LOG_DEBUG("|$%02x|$%04x|\t%-7s $%02x", thread.thread_id, thread.current_pc, "show", src);

    _engine.showPage(src);
}

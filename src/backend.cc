...
public: // public backend interface
    ...
    virtual auto updateScreen(const Page& page, const Page& page0, const Page& page1, const Page& page2, const Page& page3, const Palette& palette) -> void override final;
...
auto BackendImpl::updateScreen(const Page& page, const Page& page0, const Page& page1, const Page& page2, const Page& page3, const Palette& palette) -> void
{
    ...
    auto render_std = [&](const TextureLocker& texture) -> void
    {
    ...
    auto render_crt = [&](const TextureLocker& texture) -> void
    {
        const uint16_t screen_w = SCREEN_P;
        const uint16_t screen_h = SCREEN_H;
        const uint8_t* srcbuf   = page.data;
    ...
    auto render_all = [&](const TextureLocker& texture) -> void
    {
        const uint16_t screen_w = SCREEN_P;
        const uint16_t screen_h = SCREEN_H;
        const uint8_t* pg0buf   = page0.data;
        const uint8_t* pg1buf   = page1.data;
        const uint8_t* pg2buf   = page2.data;
        const uint8_t* pg3buf   = page3.data;
        uint8_t*       dstbuf   = texture.pixels();
        int            pitch    = texture.pitch();
        for(int y = screen_h; y != 0; --y) {
            uint8_t* dstptr = dstbuf;
            /* render page0 */ {
                const uint8_t* srcptr = pg0buf;
                for(int x = screen_w; x != 0; --x) {
                    const uint8_t colors = *srcptr++;
                    const auto c1(palette.data[(colors >> 4) & 0xf]);
                    const auto c2(palette.data[(colors >> 0) & 0xf]);
    ...
    auto render_pal = [&](const TextureLocker& texture) -> void
    {
        const uint16_t screen_w = SCREEN_W;
        const uint16_t screen_h = SCREEN_H;
        uint8_t*       dstbuf   = texture.pixels();
        int            pitch    = texture.pitch();
        /* adjust dstbuf */ {
            dstbuf += 14 * (screen_h / 8) * pitch;
        }
        /* palette low */ {
            const uint16_t palette_w = screen_w * 2;
            const uint16_t palette_h = screen_h / 8;
            for(int y = 0; y != palette_h; ++y) {
                uint8_t* dstptr = dstbuf;
                for(int x = 0; x != palette_w; ++x) {
                    const auto color(palette.data[(0 + (x / 80)) & 0xf]);
                    *dstptr++ = color.r; *dstptr++ = color.g; *dstptr++ = color.b;
                }
                dstbuf += pitch;
            }
        }
        /* palette high */ {
            ...
            for(int y = 0; y != palette_h; ++y) {
                uint8_t* dstptr = dstbuf;
                for(int x = 0; x != palette_w; ++x) {
                    const auto color(palette.data[(8 + (x / 80)) & 0xf]);
                    *dstptr++ = color.r; *dstptr++ = color.g; *dstptr++ = color.b;
                }
                dstbuf += pitch;
            }
...
auto BackendImpl::resize() -> void
{
    auto resize_window = [&]() -> void
    {
        ...
        if(bool(_window) != false) {
            ::SDL_SetWindowSize(_window.get(), dim_w, dim_h);
            ::SDL_SetWindowPosition(_window.get(), pos_x, pos_y);
        }
    };

    auto resize_all = [&]() -> void
    {
        LOG_DEBUG("resizing...");
        resize_window();
        LOG_DEBUG("resized!");

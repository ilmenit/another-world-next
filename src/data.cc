            ...
            if(resources != nullptr) {
                ...
                while(true) {
                    if(file.read(buffer, 20) != false) {
                        ...
                        resource->bank_id       = data.fetchByte();
                        resource->bank_offset   = data.fetchLongBE();
                        resource->unused4       = data.fetchWordBE();
                        resource->packed_size   = data.fetchWordBE();
                        resource->unused5       = data.fetchWordBE();
                        resource->unpacked_size = data.fetchWordBE();
...
auto MemList::dumpMemList(Resource* resources) -> bool
{
    ...
    {
        LOG_DEBUG("| 0x%02x | %-8s | %7d bytes | %7d bytes | %6.2f%% |", resource.id, type, resource.packed_size, resource.unpacked_size, gain(resource.packed_size, resource.unpacked_size));
    };
    ...
    auto log_all_resources = [&]() -> void
    {
        ...
        while(resource->type != RT_END) {
            ...
            total.count    += 1;
            total.packed   += resource->packed_size;
            total.unpacked += resource->unpacked_size;
            switch(resource->type) {
                case RT_SOUND:
                    type = "SOUND";
                    sound.count    += 1;
                    sound.packed   += resource->packed_size;
                    sound.unpacked += resource->unpacked_size;
                    break;
                case RT_MUSIC:
                    type = "MUSIC";
                    music.count    += 1;
                    music.packed   += resource->packed_size;
                    music.unpacked += resource->unpacked_size;
                    break;
                case RT_BITMAP:
                    type = "BITMAP";
                    bitmap.count    += 1;
                    bitmap.packed   += resource->packed_size;
                    bitmap.unpacked += resource->unpacked_size;
                    break;
                case RT_PALETTE:
                    type = "PALETTE";
                    palette.count    += 1;
                    palette.packed   += resource->packed_size;
                    palette.unpacked += resource->unpacked_size;
                    break;
                case RT_BYTECODE:
                    type = "BYTECODE";
                    bytecode.count    += 1;
                    bytecode.packed   += resource->packed_size;
                    bytecode.unpacked += resource->unpacked_size;
                    break;
                case RT_POLYGON1:
                    type = "POLYGON1";
                    polygon1.count    += 1;
                    polygon1.packed   += resource->packed_size;
                    polygon1.unpacked += resource->unpacked_size;
                    break;
                case RT_POLYGON2:
                    type = "POLYGON2";
                    polygon2.count    += 1;
                    polygon2.packed   += resource->packed_size;
                    polygon2.unpacked += resource->unpacked_size;
                    break;
                default:
                    type = "UNKNOWN";
                    unknown.count    += 1;
                    unknown.packed   += resource->packed_size;
                    unknown.unpacked += resource->unpacked_size;
                    break;
            }
...
auto MemList::loadResource(Resource& resource) -> bool
{
    ...
    if(_dataDir.empty() == false) {
        (void) ::snprintf(path, sizeof(path), "%s/BANK%02X", _dataDir.c_str(), resource.bank_id);
    }
    if(path[0] != '\0') {
        File file("stdio");
        if(file.open(path, "rb") != false) {
            if(file.seek(resource.bank_offset) == false) {
                return false;
            }
            if(file.read(resource.data, resource.packed_size) == false) {
                return false;
            }
            if(resource.packed_size != resource.unpacked_size) {
                ByteKiller bytekiller(resource.data, resource.packed_size, resource.unpacked_size);
...
auto MemList::dumpResource(const Resource& resource) -> void
{
    ...
    if(path[0] != '\0') {
        ...
        if(file.open(path, "wb") != false) {
            file.write(resource.data, resource.unpacked_size);
        }
    }
}
